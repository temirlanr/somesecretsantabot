...
def wishlist_handler(update: Update, context: CallbackContext) -> int:
    ...
    cur.execute(f"INSERT INTO main_info VALUES ({user.id}, {str(user.username)}, {'NULL'}, {str(update.message.text)});")
...
def define_name(update: Update, context: CallbackContext) -> int:
    ...
    cur.execute(f"UPDATE main_info SET name = {str(update.message.text)} WHERE user_id = {user.id};")
